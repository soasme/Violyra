...
const { upscaleVideo, parseTargetResolution } = require("./upscale.js");

const CLI_SCRIPT_PATH = ".agents/skills/video-upscale/scripts/compile_full_song.js";
...
const DEFAULT_WORK_DIR = "assets/final/build-1080";
...
function buildCoverFilter({ setptsFactor, width, height, fps }) {
  return [
...
async function main() {
  ...
  for (let index = 0; index < scenes.length; index += 1) {
    ...
    console.log(
      `Stretching scene ${sceneId} to ${timing.duration.toFixed(3)}s and fitting ${outputWidth}x${outputHeight}`
    );
    ...
    concatLines.push(`file '${stretchedPath.replace(/'/g, "'\\''")}'`);
