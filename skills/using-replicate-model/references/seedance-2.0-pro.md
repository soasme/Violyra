...
Based on Seedance 1.5 Pro (`bytedance/seedance-1.5-pro`) patterns:
...
| `fps`             | integer | `24`     | `24` only                                            | Frame rate                                                          |
...
> Before using, verify the actual schema:
> ```bash
> curl -s "https://api.replicate.com/v1/models/bytedance/seedance-2.0-pro" \
...
PREDICTION=$(curl -s -X POST \
  "https://api.replicate.com/v1/models/bytedance/seedance-2.0-pro/predictions" \
  -H "Authorization: Bearer $REPLICATE_API_TOKEN" \
...
async function generateVideo(input) {
  ...
  const createRes = await fetch(
    `${BASE}/models/bytedance/seedance-2.0-pro/predictions`,
    {
      method: "POST",
      headers: {
...
- These examples are based on Seedance 1.5 Pro patterns. **Verify the live schema before use.**
- `aspect_ratio` is likely ignored when `image` is provided (consistent with 1.5 Pro behavior).
